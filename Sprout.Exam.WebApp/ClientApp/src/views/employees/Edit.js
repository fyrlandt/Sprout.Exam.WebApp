...
<button type="button" onClick={() => this.props.history.push("/employees/index")} className="btn btn-primary">Back</button>
  ...
  async saveEmployee() {
    var validationResult = ValidationUtil.validateEmployeeFields(this.state);
      if (validationResult != "")
  ...
  async getEmployee(id) {
    ...
    const response = await fetch('api/employees/' + id, {
      ...
      var birthdate = DateUtil.dateTimeToDate(data.birthdate)
      this.setState({ id: data.id, fullName: data.fullName, birthdate: birthdate, tin: data.tin, typeId: data.employeeTypeId, loading: false,loadingSave: false });
  }
}
